...
import { ElementRef, Fragment } from "react";
...
export const ChatMessages = ({name,member,chatId,apiUrl,socketUrl,socketQuery,paramKey,paramValue,type}:ChatMessagesProps) => {
  ...
  const {data,fetchNextPage,hasNextPage,status,isFetchingNextPage}=useChatQuery({queryKey,apiUrl,paramKey,paramValue})
  ...
  return <div className="flex flex-col flex-1 justify-center items-center">
    ...
    <p className="text-xs text-zinc-500 dark:text-zinc-400">Loading messages
    </p>
  </div>
  else if(status==='error' ) 
  return <div className="flex flex-col flex-1 justify-center items-center">
    <ServerCrash className="w-7 h-7 text-zinc-500 my-4" />
    <p className="text-xs text-zinc-500 dark:text-zinc-400">something went wrong
    </p>
  </div>
